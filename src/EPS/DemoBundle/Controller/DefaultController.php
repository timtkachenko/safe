...
class DefaultController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $qb = $em->createQueryBuilder()->from('NXCSafeBundle:Whisper', 'p')->leftJoin('p.user', 'c')
                 ->select('p.id, p.whisper, p.created, c.username, c.slug')->groupBy('p.id');
...
//        $grid->addColumn('Author', array('name' => 'name', 'index' => 'a.name', 'width' => '150', 'stype' => 'select', 'searchoptions' => array('value' => $lstauthor)));
        $grid->addColumn('whisper', array('name' => 'whisper', 'index' => 'p.whisper', 'autocomplete' => 'ajax_whisper', 'width' => '150','editable'=>true,"editoptions"=>array("size"=>10, "maxlength"=>15)));
